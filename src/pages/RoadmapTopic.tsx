      ...
      <main className="pt-16">
        <div className="max-w-4xl mx-auto px-4 py-10 md:py-14">
          ...
          <div className="mt-6 flex items-center justify-between gap-4">
            {previous ? <Link to={`/roadmap/${previous[0]}`} className="max-w-[48%] inline-flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 hover:text-aicc-purple"><ArrowLeft className="w-4 h-4 shrink-0" /><span className="truncate">{previous[1]}</span></Link> : <span />}
            {next ? <Link to={`/roadmap/${next[0]}`} className="max-w-[48%] inline-flex items-center gap-2 text-sm font-semibold text-aicc-purple dark:text-aicc-purple-light hover:underline"><span className="truncate">{next[1]}</span><ArrowRight className="w-4 h-4 shrink-0" /></Link> : <span />}
          </div>
        </div>
      </main>
      <Footer />
